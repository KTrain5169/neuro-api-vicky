...
describe('main.run()', () => {
  ...
  test('A) No packet-list provided: server starts, runTest succeeds, outputs set', async () => {
    (core.getInput as jest.Mock).mockImplementation((name: string) => {
      switch (name) {
        ...
        case 'testFile': return 'test-file.js';
        case 'runner': return 'node';
        case 'packet-list': return '';
        default: return '';
      }
    });
    ...
    (runnerModule.runTest as jest.Mock).mockResolvedValue({ success: true, durationMs: 123 });

    await run();

    expect(serverModule.startServer).toHaveBeenCalledTimes(1);
    const [portArg, loggerArg, packetListArg] = (serverModule.startServer as jest.Mock).mock.calls[0];
    ...
    expect(runnerModule.runTest).toHaveBeenCalledWith('node', 'test-file.js', loggerArg);
  ...
  test('B) packet-list provided but file does NOT exist → setFailed called early', async () => {
    (core.getInput as jest.Mock).mockImplementation((name: string) => {
      switch (name) {
        ...
        case 'testFile': return 'ignore.js';
        case 'runner': return 'node';
        case 'packet-list': return 'nonexistent.json';
        default: return '';
      }
    });

    existsSyncMock.mockImplementation((p: any) => p.toString() !== 'nonexistent.json');
  ...
  test('C) packet-list provided but invalid JSON → setFailed called with parse error', async () => {
    (core.getInput as jest.Mock).mockImplementation((name: string) => {
      switch (name) {
        case 'port': return '5000';
        case 'testFile': return 'ignore2.js';
        case 'runner': return 'node';
        case 'packet-list': return 'bad.json';
        default: return '';
      }
    });
    ...
    .mockImplementation((p: any) => (p.toString() === 'bad.json' ? '{ invalid JSON ' : ''));
