...
describe('startServer/stopServer', () => {
  ...
  test('processes a sequence of valid packets and logs expected lines', (done) => {
    ...
    const fixturePath = path.join(__dirname, '../__fixtures__/packets.valid.json');
    ...
    wss.on('listening', () => {
      ...
      client.on('close', () => {
        ...
        setTimeout(() => {
          ...
          expect(contentsJoined).toMatch(/Starting WebSocket server on port \d+/);
          ...
          expect(contentsJoined).toMatch(/Force request:.*"query":"doSomething"/);
          expect(contentsJoined).toMatch(/Shutdown ready command packet received/);
  ...
  test('exits on invalid packet (no startup)', (done) => {
    ...
    const fixturePath = path.join(__dirname, '../__fixtures__/packets.invalid.json');
    const raw = fs.readFileSync(fixturePath, 'utf8');
    const packetList = JSON.parse(raw);

    // 2) Start a fresh server & logger in another temp subdir
    const subTmp = fs.mkdtempSync(path.join(os.tmpdir(), 'vicky-server-invalid-'));
